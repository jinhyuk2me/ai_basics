## TL;DR
- AI 기초 학습은 수학 → 프로그래밍 → 머신러닝 순으로 단계적으로 진행합니다.
- 9단계 로드맵을 따라 필수 개념을 익히고, 각 단계마다 실습을 병행합니다.
...
| 3 | 프로그래밍 기초 | Python과 자료구조를 어떻게 활용하나? | [[Programming/1. 파이썬 기초]] | NumPy, 자료구조, 알고리즘 복잡도 |
| 4 | ML 기초 개념 | 지도/비지도/평가 지표는 무엇인가? | [[MLDL Basic/1. 학습 패러다임]] | 훈련/검증/테스트, 손실 함수, 평가 지표 |
| 5 | 최적화/정규화 | 학습 절차를 어떻게 안정화하나? | [[MLDL Basic/3. 최적화]] | SGD, Adam, 배치 정규화, 드롭아웃 |
| 6 | 신경망 기초 | 심층 신경망은 어떻게 동작하나? | [[MLDL Basic/5. 신경망 기본 구조]] | MLP, 활성화 함수, 역전파 |
| 7 | 신경망 아키텍처 | CNN, RNN은 어떻게 동작하나? | [[MLDL Basic/8. 합성곱 신경망 기초]] | 합성곱, 풀링, 시퀀스 모델 |
| 8 | 고급 수학 | 행렬 분해와 합성곱 이론은? | [[Math/Linear Algebra/8. 선형 연산자]] | 선형 변환, 행렬 분해, 합성곱과 FFT |
| 9 | 분석/수치 심화 | 라플라스·푸리에·측도가 DL과 어떻게 연결되나? | [[Math/Analysis/1. 라플라스 변환과 미분방정식]] | Neural ODE, Bayesian DL, Spectral CNN |
...
**다음으로**: 이론 준비가 끝나면 3단계(프로그래밍)로 이동

---

### 3단계: 프로그래밍 기초

**학습 목표**: Python과 자료구조로 효율적인 코드 작성

**필수 노트**:
- [[Programming/1. 파이썬 기초]]: 문법, NumPy, Pandas
- [[Programming/2. 자료구조와 알고리즘]]: 리스트, 딕셔너리, 시간 복잡도

**왜 필요한가?**
- 데이터 전처리와 모델 구현에 필수
- 알고리즘 효율성 이해로 대규모 데이터 처리
- PyTorch/TensorFlow는 Python 기반

**실습 포인트**:
- NumPy 배열 연산으로 벡터/행렬 조작
- Pandas로 CSV 데이터 로드 및 전처리
- 간단한 알고리즘 시간 복잡도 분석

**다음으로**: 코딩 준비가 끝나면 4단계(ML 기초)로 이동
...
**다음으로**: ML 개념이 잡히면 5단계(최적화/정규화)로 이동
...
**다음으로**: 최적화 기법을 익히면 6단계(신경망 기초)로 이동
...
- [[MLDL Basic/PyTorch Tensor]]: Tensor 기본 연산
...
**다음으로**: 기본 신경망을 이해하면 7단계(CNN/RNN)로 이동
...
**다음으로**: 주요 아키텍처를 익히면 8단계(연산자)로 이동
...
**다음으로**: 기초 과정 완료. 9단계(심화) 또는 프로젝트로 이동
...
### 3단계 완료 시
- [ ] NumPy로 배열 연산을 능숙하게 수행할 수 있다
- [ ] 시간 복잡도를 분석할 수 있다
- [ ] Pandas로 데이터 전처리를 할 수 있다
...
### 최소 경로 (빠른 시작)
1단계 → 3단계 → 4단계 → 5단계 → 6단계 → 프로젝트

**특징**: 수학을 최소화하고 실습 위주로 빠르게 진행
**적합한 사람**: 코딩 경험이 있고 빠르게 모델을 만들어보고 싶은 사람
