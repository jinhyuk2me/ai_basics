...
- **지도 학습 (Supervised Learning)**
  - 입력-정답 쌍 `(x, y)`를 사용하여 함수 `f: x → y`를 학습.
  - 과제: 분류, 회귀, 시계열 예측, 구조적 예측(시퀀스 라벨링 등).
  - 대표 모델: Logistic Regression, SVM, Random Forest, CNN, Transformer.
  - 응용: 이미지 분류, 스팸 필터링, 수요 예측.
- **비지도 학습 (Unsupervised Learning)**
  - 라벨 없이 데이터의 구조를 파악. 패턴 발견, 군집화, 차원 축소.
  - 대표 기법: K-means, GMM, PCA, Autoencoder, Density Estimation.
  - 응용: 고객 세분화, 이상 탐지, 데이터 시각화.
- **자기지도 학습 (Self-supervised Learning)**
  - 라벨 없이도 프리텍스트 태스크를 정의해 표현 벡터를 학습.
  - 예: 마스크 언어 모델(BERT), 이미지 색 복원, contrastive learning(SimCLR).
  - 라벨이 적더라도 사전학습 후 파인튜닝으로 좋은 성능 달성.
- **반지도 학습 (Semi-supervised Learning)**
  - 소량의 라벨 + 대량의 비라벨 데이터를 동시에 활용.
  - Consistency regularization, pseudo-labeling, graph-based methods.
  - 응용: 의료 영상, 라벨링 비용이 큰 도메인.
- **강화 학습 (Reinforcement Learning)**
  - 에이전트가 환경과 상호작용하며 보상을 최대화하는 정책을 학습.
  - 구성요소: 상태 S, 행동 A, 보상 R, 전이 T, 정책 π, 가치 함수 V.
  - 대표 알고리즘: Q-learning, Policy Gradient, Actor-Critic, PPO.
  - 응용: 게임, 로보틱스, 추천 시스템.
- **온라인/배치 학습**
  - 온라인 학습: 데이터 스트림을 순차적으로 처리, 모델이 지속적으로 업데이트.
  - 배치 학습: 고정된 데이터셋을 한 번에 학습, 모델 업데이트 주기가 길다.
