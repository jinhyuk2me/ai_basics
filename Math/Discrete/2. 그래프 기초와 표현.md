...
그래프가 얼마나 잘 '연결'되어 있는지를 나타내는 속성입니다. 이는 네트워크의 안정성, 정보 전파 속도 등과 직결되는 중요한 지표입니다.

**연결 그래프 (Connected Graph)**:
- **정의**: 그래프의 임의의 두 노드 $u, v$에 대해 $u$에서 $v$로 가는 경로(path)가 항상 존재하는 무방향 그래프.
- **직관적 설명**: 그래프가 하나의 '덩어리'로 이루어져 있어, 어떤 노드에서든 다른 모든 노드로 이동할 수 있음을 의미합니다. 분리된 부분이 없습니다.

**연결 요소 (Connected Component)**:
- **정의**: 연결 그래프의 '최대' 부분 그래프. 즉, 연결 요소에 속한 노드를 추가하면 더 이상 연결 그래프가 되지 않는 부분 그래프입니다.
- **직관적 설명**: 전체 그래프가 여러 덩어리로 나뉘어 있을 때, 그 각 덩어리 하나하나를 의미합니다.
- **예시**:
  ```
  그래프 1: 1-2-3  (연결 요소 1개)
  그래프 2: 1-2, 3-4 (연결 요소 2개: {1,2}, {3,4})
  ```

**강연결 (Strongly Connected, for directed)**:
- **정의**: 방향 그래프에서 임의의 두 노드 $u, v$에 대해, $u$에서 $v$로 가는 경로와 $v$에서 $u$로 가는 경로가 **모두** 존재하는 경우.
- **직관적 설명**: 방향 그래프의 어떤 노드에서 출발하든 다른 모든 노드를 방문하고 다시 출발점으로 돌아올 수 있는 '강력한 순환' 구조를 가집니다.
- **예시**:
  ```
  강연결:    1 <-> 2
  강연결 아님: 1 -> 2 (2에서 1로 돌아올 수 없음)
  ```

**거리 (Distance)**:
- **정의**: 두 노드 $u, v$ 사이의 최단 경로 길이. $d(u, v)$로 표기합니다.
- **수식**:
  $$
...
**직경 (Diameter)**:
- **정의**: 그래프 내 모든 노드 쌍 간의 거리 중 최댓값. 즉, 그래프에서 가장 멀리 떨어진 두 노드 사이의 거리입니다.
- **수식**:
  $$
  \text{diam}(G) = \max_{u, v \in V} d(u, v)
  $$
- **의미**: 직경이 작을수록 그래프의 정보 전파 속도가 빠르다고 해석할 수 있습니다.

---
#### 심화: 그래프 컷 (Graph Cuts)
연결성을 끊기 위해 필요한 최소한의 '비용'을 의미하며, 네트워크의 취약점 분석 등에 사용됩니다.

**정점 컷 (Vertex Cut)**:
- **정의**: 제거했을 때 그래프의 연결 요소 개수가 늘어나는 노드들의 집합.
- **예시**: `A - B - C` 그래프에서 `{B}`는 정점 컷입니다.

**간선 컷 (Edge Cut)**:
- **정의**: 제거했을 때 그래프의 연결 요소 개수가 늘어나는 엣지들의 집합.
	- **예시**: `A - B - C` 그래프에서 `{ (A,B) }` 또는 `{ (B,C) }`는 간선 컷입니다.

---
#### 실습: NetworkX로 연결성 확인하기
...
# 1. 연결성 확인
print(f"Is G connected? {nx.is_connected(G)}") # False
...
# 방향 그래프 생성
DG = nx.DiGraph()
DG.add_edges_from([(1, 2), (2, 3), (3, 1), (4, 1)])

# 3. 강연결성 확인
print(f"\nIs DG strongly connected? {nx.is_strongly_connected(DG)}") # False
...
1. **그래프 알고리즘**
   - [[3. 그래프 알고리즘|그래프 알고리즘 상세 노트]]
   - BFS, DFS, Dijkstra, MST

2. **스펙트럴 이론과 GNN**
   - [[4. 스펙트럴 그래프 이론과 GNN|스펙트럴 그래프와 GNN]]
   - Laplacian, GCN, message passing

3. **선형대수 연결**
   - [[Math/Linear Algebra/4. 고유값과 고유벡터|고유값]]
   - 인접 행렬의 고유값과 그래프 특성
