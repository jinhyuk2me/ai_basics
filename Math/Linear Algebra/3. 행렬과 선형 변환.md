...
행렬식 $\det(A)$는 선형 변환이 단위 부피(혹은 면적)를 얼마나 바꾸는가를 나타내는 값이다.

$$
|\det(A)| = \text{부피(면적) 스케일 비율}, \quad \text{sign}(\det(A)) = \text{방향(orientation) 유지 여부}
$$

이 관점을 바탕으로 행렬식의 주요 성질들을 이해할 수 있다.
...
**기하학적 의미**:
- 전치는 "행 ↔ 열"을 바꾸는 것이고, 기하학적으로는 벡터 집합의 순서를 바꾸는 것에 불과하다
- 행벡터는 평면 위의 두 벡터, 열벡터는 그 평면의 축 방향 기저로 볼 수 있다
- 전치를 해도 그 평행사변형(혹은 평행육면체)의 모양과 부피는 변하지 않는다
- 따라서 부피(면적) 스케일 비율은 동일함
...
**기하학적 의미**:
- 행렬 $A$와 $B$는 각각 공간을 변환하는 선형 변환이다
- $B$: 먼저 공간을 변환 → 부피가 $|\det(B)|$배 됨
- $A$: 그 결과를 다시 변환 → 부피가 $|\det(A)|$배 더 됨
- 따라서 전체 변환의 부피 비율은 $|\det(A)| \times |\det(B)|$
- 즉, 연속된 두 변환의 부피 비율은 곱으로 누적된다
...
**기하학적 의미**:
- $A$가 $2 \times 2$라면 2D 면적, $3 \times 3$이면 3D 부피다
- 모든 행(또는 모든 열)에 스칼라 $k$를 곱하는 것은 공간의 각 축을 $k$배 늘리는 것과 같다
- 2D: 면적이 $k^2$배
- 3D: 부피가 $k^3$배
- 일반적으로 $n$차원: $k^n$배
...
두 행을 바꾸면 $\det(A) \rightarrow -\det(A)$

**기하학적 의미**:
- 두 행(즉 두 기저 벡터)을 바꾸는 것은 좌표계의 방향(orientation)을 뒤집는 것이다
- 오른손 좌표계 → 왼손 좌표계
- 즉, "부피의 부호"만 바뀜
- 실제 크기(절댓값)는 변하지 않음
...
한 행에 스칼라 $k$를 곱하면 $\det(A) \rightarrow k\det(A)$

**기하학적 의미**:
- 한 행(기저 벡터)을 $k$배 늘리면, 평행사변형(또는 평행육면체)의 한 변이 $k$배가 된다
- 즉, 부피(면적)도 $k$배가 된다
- 전체 부피 = (밑변 × 높이 × ...)이므로, 한 변만 $k$배 → 전체 부피도 $k$배

**6. 행 덧셈 (Row Addition)**

한 행에 다른 행의 배수를 더해도 $\det(A)$는 변하지 않는다.

**기하학적 의미**:
- 한 벡터를 다른 벡터의 방향으로 조금 "기울인다"고 해서 평행사변형의 면적(혹은 부피)은 변하지 않는다
- 예를 들어, 평면에서 $(1, 0)$, $(0, 1)$이 만드는 정사각형에 두 번째 벡터를 $(1, 1)$로 바꾼다고 해보자
- 이것은 단지 "평행사변형의 모양이 비틀린 것"일 뿐, 면적은 그대로다

### 4.5 행렬식 성질 요약표

| 성질 | 식 | 기하학적 의미 |
|------|-----|--------------|
| 전치 | $\det(A^T) = \det(A)$ | 같은 부피, 방향 변화 없음 |
| 곱 | $\det(AB) = \det(A)\det(B)$ | 연속 변환 시 부피 비율이 곱으로 누적됨 |
| 스칼라 곱 | $\det(kA) = k^n \det(A)$ | 모든 축을 $k$배 확장 → 부피는 $k^n$배 |
| 행 교환 | 행 2개 교환 → 부호 반전 | 좌표계 방향(orientation) 뒤집힘 |
| 행 배수 | 한 행에 $k$배 → det도 $k$배 | 한 변 길이 $k$배 확장 |
| 행 덧셈 | 한 행에 다른 행의 배수 더함 → 불변 | 평행이동/기울기 변화는 부피에 영향 없음 |

**핵심**: 행렬식의 모든 성질은 "부피와 방향(orientation)"의 보존 혹은 변화로 이해된다.
